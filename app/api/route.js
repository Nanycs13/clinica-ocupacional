...
// Dica de Engenharia: Em Next.js dev, isso evita criar múltiplas conexões
...
export async function GET() {
  try {
    // ---------------------------------------------------------
    // 1. CARREGAR DADOS BRUTOS NECESSÁRIOS
    // ---------------------------------------------------------
    
    // Buscamos todos os exames para calcular os KPIs globais
    // (Em um sistema real com milhões de linhas, usaríamos count() separado para performance)
    const todosExames = await prisma.exame.findMany();

    // ---------------------------------------------------------
    // 2. PROCESSAMENTO: KPIS GERAIS
    // ---------------------------------------------------------
    const totalExames = todosExames.length;
    const examesComAfastamento = todosExames.filter(e => e.afastamento?.trim().toLowerCase() === 'sim');
